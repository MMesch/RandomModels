...
def main():
    ...
    dtrans = [0.,nz/12.,nz/6.,0.]
    partition = make_partition(dlocs,dtrans,np.arange(nz))
    hscales = np.zeros( (nz) )
    vscales = np.zeros( (nz) )
    hscales += 2.*PI*5./sizez*partition[0]
    hscales += 2.*PI*2./sizez*partition[1]
    hscales += 2.*PI*5./sizez*partition[2]
    plt.figure()
    plt.plot(hscales)
    vscales += 2.*PI*1./sizez*partition[0]
    vscales += 2.*PI*20./sizez*partition[1]
    vscales += 2.*PI*2./sizez*partition[2]
    plt.figure()
    plt.plot(hscales)
    ...
    rhomax = 2*max(hscales)
    for ikx,kx in enumerate(kxs[:nkx/2+1]):
        for iky,ky in enumerate(kys):
            rho = np.sqrt(kx**2+ky**2)
            if rho>rhomax: continue
            print 'point {:d}/{:d}'.format(ikx*nky+iky,nkx/2*nky)
            ...
            w_red = w[mask]
            nbasis = len(w_red)
            L = np.sqrt(w_red).reshape(1,nbasis)*E[:,mask]
            coeffs_buf = np.empty( nbasis, dtype=np.complex)
            #create uncorrelated Fourier coefficients with unit power for every layer:
            coeffs_buf.real = np.random.normal(loc=0,scale=1,size=nbasis)
            coeffs_buf.imag = np.random.normal(loc=0,scale=1,size=nbasis)
    ...
    #---- output VTK model ----
    gridToVTK('vtk/Layered',zcoords,xcoords,ycoords,pointData= {'random field': model})
